...
5. **`nativewind` di `apps/mobile/package.json` DIKUNCI ke `4.1.23`, jangan pakai `^4.0.0` atau range apa pun.** nativewind `4.2.0`–`4.2.6` punya bug dari `react-native-css-interop` yang butuh package `react-native-worklets` yang tidak pernah terinstall → build/bundling gagal dengan error `Cannot find module 'react-native-worklets/plugin'`. Ini bug upstream yang dikonfirmasi banyak orang (lihat GitHub issue nativewind #1574, #1580), bukan salah konfigurasi kita. Jangan `pnpm update nativewind` atau ubah ke `^4.x` sampai nativewind 5 stabil rilis atau bug ini resmi di-fix di 4.2.x.
